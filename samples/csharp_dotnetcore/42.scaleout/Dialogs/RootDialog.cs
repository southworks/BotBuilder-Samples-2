...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class RootDialog : ComponentDialog
    {
        ...
        {
            return await stepContext.PromptAsync("number", new PromptOptions { Prompt = MessageFactory.Text("Enter a number.") }, cancellationToken).ConfigureAwait(false);
        }
        ...
        private static async Task<DialogTurnResult> Step2Async(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync("number", new PromptOptions { Prompt = MessageFactory.Text($"I have {first} now enter another number") }, cancellationToken).ConfigureAwait(false);
        }
        ...
        private static async Task<DialogTurnResult> Step3Async(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text($"The result of the first minus the second is {first - second}."), cancellationToken).ConfigureAwait(false);
